...
/***/ function(module, exports, __webpack_require__) {
	...
	Component.options.__file = "/58fe/project/fang-booking/src/views/detailRenter/index.vue"
	if (Component.esModule && Object.keys(Component.esModule).some(function (key) {return key !== "default" && key !== "__esModule"})) {console.error("named exports are not supported in *.vue files.")}
	...
	if (false) {(function () {
	  ...
	  module.hot.accept()
	  if (!module.hot.data) {
	    hotAPI.createRecord("data-v-02530d77", Component.options)
	  } else {
	    hotAPI.reload("data-v-02530d77", Component.options)
	  }
	})()}
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _dataWxPay = __webpack_require__(232);

	var _dataWxPay2 = _interopRequireDefault(_dataWxPay);

	var _statusButtonEnum = __webpack_require__(233);
	...
	exports.default = {
	    ...
	    data: function data() {
	        return {
	            ...
	            /*房东信息数据*/
	            dataPerson: (0, _extends3.default)(_dataPerson2.default, { role: 2 }),
	            /*帖子信息数据*/
	    ...
	    methods: {
	        ...
	        getOrderInfoByIdCb: function getOrderInfoByIdCb(state, res) {
	            ...
	            if (res.code == 0 && res.data) {
	                ...
	                var _res$data$sellerId = _res$data.sellerId;
	                _this.dataPerson.userid = _res$data$sellerId === undefined ? '' : _res$data$sellerId;
	                var _res$data$orderDetail = _res$data.orderDetail;
	                var _res$data$orderDetail2 = _res$data$orderDetail.sellerPic;
	                _this.dataPerson.pic = _res$data$orderDetail2 === undefined ? '' : _res$data$orderDetail2;
	                var _res$data$orderDetail3 = _res$data$orderDetail.sellerName;
	                _this.dataPerson.name = _res$data$orderDetail3 === undefined ? '' : _res$data$orderDetail3;
	                var _res$data$orderDetail4 = _res$data$orderDetail.sellerPhone;
	                _this.dataPerson.phone = _res$data$orderDetail4 === undefined ? '' : _res$data$orderDetail4;
	                ...
	                var _res$data$orderDetail5 = _res$data.orderDetail.checkInDate;
	                _this.dataBooking.checkInDate = _res$data$orderDetail5 === undefined ? '' : _res$data$orderDetail5;
	                ...
	                var _res$data$orderId2 = _res$data.orderId;
	                _this.dataWxPay.orderNo = _res$data$orderId2 === undefined ? '' : _res$data$orderId2;
	                var _res$data$bizCode = _res$data.bizCode;
	                _this.dataWxPay.bizCode = _res$data$bizCode === undefined ? '' : _res$data$bizCode;
	                var _res$data$sign = _res$data.sign;
	                _this.dataWxPay.sign = _res$data$sign === undefined ? '' : _res$data$sign;
	                var _res$data$tid = _res$data.tid;
	                _this.dataWxPay.tid = _res$data$tid === undefined ? '' : _res$data$tid;
	                var _res$data$buyerPayDea2 = _res$data.buyerPayDeadline;
	                _this.dataWxPay.orderExpireTime = _res$data$buyerPayDea2 === undefined ? '' : _res$data$buyerPayDea2;
	            } else if (res.code == 1000007) {
	                //用户未登陆
	                this.userLogin();
	            } else {
	        ...
	        handlerClickButton: function handlerClickButton(dataButton) {
	            ...
	            switch (_action) {
	                ...
	                case 'pay':
	                    //付款
	                    //版本控制
	                    var _ref = [_this.$app.getAppVersion(), _this.$app.getAppVersionOn()],
	                        _currentAppVersion = _ref[0],
	                        _onAppVersion = _ref[1];

	                    var isEgt = _this.$util.compareEgt(_currentAppVersion, _onAppVersion);
	                    if (!isEgt) {
	                        _this.$app.showDialog("single", "版本过低", "升级58app至最新版本才可微信付款哦~", function (index) {}, "好");
	                    } else {
	                        _this.payOrder();
	                    }
	                    break;
	                case 'refund':
	                    ...
	                    if (dataButton.dialog) {
	                        ...
	                        _this.$app.showDialog('double', _dialog2.title, _dialog2.content, _this.applyRefund, _dialog2.text1, _dialog2.text2);
	                    }
	                    break;
	                case 'checkIn':
	                    //确认入住
	                    if (dataButton.dialog) {
	                        ...
	                        var _content = _dialog3.content.replace("#name#", _this.dataPerson.name);
	                        (0, _extends3.default)(_this.dataDialogCommon, {
	                            show: true,
	                            action: _action,
	                            title: _dialog3.title,
	                            content: _content,
	                            buttons: [_dialog3.text1, _dialog3.text2]
	                        });
	                    }
	                    break;
	                case 'arbitration':
	         ...
	         * 操作按钮: 租客申请退款操作
	         */
	        applyRefund: function applyRefund(index) {
	            if (index == 1) {
	                this.buttonRequest(_api2.default.applyRefund.url);
	            }
	        },
	         ...
	         * 操作按钮: 租客确认入住操作
	         */
	        confirmOrder: function confirmOrder(index) {
	            if (index == 1) {
	         ...
	         * 按钮操作: 确认入住
	         */
	        handlerClickButtonDialog: function handlerClickButtonDialog(action, index) {
	            var _this = this;
	            switch (action) {
	                case 'checkIn':
	                    //确认入住
	         ...
	         * requestUrl: 操作请求URL
	         */
	        buttonRequest: function buttonRequest(requestUrl) {
	            var _this = this;
	            _this.isLoading = true;
	            _this.$request.jsonp({
	                url: requestUrl,
	         ...
	         * 操作按钮: 数据请求回调
	         */
	        buttonRequestCb: function buttonRequestCb(state, res) {
	            var _this = this;
	            _this.isLoading = false;
	            if (!state) {
	        ...
	        payOrder: function payOrder() {
	            ...
	            _this.isLoading = true;
	            _this.$request.jsonp({
	                url: _api2.default.wxAppPay.url,
	                data: {
	         ...
	         * 操作按钮: 微信付款及数据请求回调
	         */
	        payOrderCb: function payOrderCb(state, res) {
	            var _this = this;
	                ...
	                callback: function callback(state, res) {
	                    ...
	                    if (res.code == 0) {
	                        if (res.data == 1) {
	                            ...
	                            //人脸认证未通过
	                            var _ref2 = [_this.$app.getAppVersion(), _this.$app.getAppVersionFaceAuth()],
	                                _appVersion = _ref2[0],
	                                _faceAuthVersion = _ref2[1];
	         ...
	         *  申请仲裁操作
	         */
	        arbitrationOrder: function arbitrationOrder() {
	            var _this = this;
	            var dataButton = _this.statusButtonEnum[9].arbitration;
	            if (dataButton.dialog) {
	                var _dialog = dataButton.dialog;
	...
	var statusButtonEnum = {
	    ...
	    9: {
	        arbitration: {
	            ...
	            loadPage: {
	                title: "",
	                url: _api2.default.plaintiff.url
	            },
	            log: 'arbitrate' //埋点
	        }
	    }
...
/***/ function(module, exports, __webpack_require__) {
	  ...
	  return _c('article', {
	    ...
	    on: {
	      "clickUser": function($event) {
	        _vm.trackLog(_vm.dataLog.person.ownerphoto)
	      },
	      "clickChat": function($event) {
	        _vm.trackLog(_vm.dataLog.person.im)
	      },
	      "clickTel": function($event) {
	        _vm.trackLog(_vm.dataLog.person.tel)
	      }
	    }
	    ...
	    on: {
	      "clickInfo": function($event) {
	        _vm.trackLog(_vm.dataLog.house.info)
	      }
	    }
	    ...
	    on: {
	      "clickButton": _vm.handlerClickButtonDialog
	    }
	  }, [_c('AgreementRadio', {
	    attrs: {
	...
	if (false) {
	  module.hot.accept()
	  if (module.hot.data) {
	     require("vue-hot-reload-api").rerender("data-v-02530d77", module.exports)
	  }
	}
