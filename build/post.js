...
/***/ function(module, exports, __webpack_require__) {
	...
	Component.options.__file = "/58fe/project/fang-booking/src/views/post/index.vue"
	if (Component.esModule && Object.keys(Component.esModule).some(function (key) {return key !== "default" && key !== "__esModule"})) {console.error("named exports are not supported in *.vue files.")}
	...
	if (false) {(function () {
	  ...
	  module.hot.accept()
	  if (!module.hot.data) {
	    hotAPI.createRecord("data-v-6fad3280", Component.options)
	  } else {
	    hotAPI.reload("data-v-6fad3280", Component.options)
	  }
	})()}
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = {
	    ...
	    data: function data() {
	        return {
	            ...
	            loadPage: {
	                ...
	                renterDetail: {
	                    title: "订单详情",
	                    url: _api2.default.pageGetRenterDetail.url //租客定金订单详情页面地址
	                }
	            },
	            /*页面高度,用于判断是否有键盘弹起*/
	    ...
	    mounted: function mounted() {
	        ...
	        this.$eventHub.$on(this.$eventAction.INPUT_FOCUS, function () {
	            _this2.dataBottomBar.barPosition = 'absolute';
	        });
	        this.$eventHub.$on(this.$eventAction.INPUT_BLUR, function () {
	            _this2.dataBottomBar.barPosition = '';
	        });
	    },
	    ...
	    methods: {
	         ...
	         * 数据请求: 获取预订房源信息
	         */
	        getPostInfoById: function getPostInfoById() {
	            var _this = this;
	            _this.isLoading = true;
	            _this.$request.jsonp({
	                url: _api2.default.getPostInfoById.url,
	                data: {
	                    infoId: _this.infoId
	                },
	         ...
	         * 数据请求: 获取预订房源信息回调
	         */
	        getPostInfoByIdCb: function getPostInfoByIdCb(state, res) {
	            var _this = this;
	            ...
	            switch (+res.code) {
	                case 0:
	                    ...
	                    if (res.data) {
	                        ...
	                        var _res$data$infoId = _res$data.infoId;
	                        _this.dataHouse.infoId = _res$data$infoId === undefined ? '' : _res$data$infoId;
	                        var _res$data$title = _res$data.title;
	                        _this.dataHouse.title = _res$data$title === undefined ? '' : _res$data$title;
	                        var _res$data$picUrl = _res$data.picUrl;
	                        _this.dataHouse.picUrl = _res$data$picUrl === undefined ? '' : _res$data$picUrl;
	                        var _res$data$area = _res$data.area;
	                        _this.dataHouse.area = _res$data$area === undefined ? '' : _res$data$area;
	                        var _res$data$pLocalName = _res$data.pLocalName;
	                        _this.dataHouse.pLocalName = _res$data$pLocalName === undefined ? '' : _res$data$pLocalName;
	                        var _res$data$pAreaLocalN = _res$data.pAreaLocalName;
	                        _this.dataHouse.pAreaLocalName = _res$data$pAreaLocalN === undefined ? '' : _res$data$pAreaLocalN;
	                        var _res$data$chaoXiang = _res$data.chaoXiang;
	                        _this.dataHouse.chaoXiang = _res$data$chaoXiang === undefined ? '' : _res$data$chaoXiang;
	                        var _res$data$price = _res$data.price;
	                        _this.dataHouse.price = _res$data$price === undefined ? '' : _res$data$price;
	                        var _res$data$nativeApp = _res$data.nativeApp;
	                        _this.dataHouse.nativeApp = _res$data$nativeApp === undefined ? '' : _res$data$nativeApp;
	                        var _res$data$buyerPhone = _res$data.buyerPhone;
	                        _this.formData.buyerPhone = _res$data$buyerPhone === undefined ? _this.formData.buyerPhone : _res$data$buyerPhone;
	                        var _res$data$nowTime = _res$data.nowTime;
	                        _this.nowTime = _res$data$nowTime === undefined ? '' : _res$data$nowTime;
	                    } else {
	                        _this.$app.showDialog("single", "提示", "服务器房源数据异常,请稍后再试!", function (index) {}, "返回");
	                    }
	                    break;
	                case 1000007:
	                    //未登录
	                    ...
	                    break;
	                default:
	                    _this.$app.showDialog("single", "提示", res.message, function (index) {}, "返回");
	                    break;
	            }
	        },
	         ...
	         * 表单项: 初始化入住日期数据源及默认日期
	         */
	        checkInDateInit: function checkInDateInit() {
	            var _this = this;
	         ...
	         */
	        extractDate: function extractDate(str) {
	            var _date = '';
	         ...
	         */
	        checkInDateDataSource: function checkInDateDataSource() {
	            var _this = this;
	            var _nowTime = _this.extractDate(_this.nowTime);
	         ...
	         * 表单项: 入住日期 默认时间
	         */
	        checkInDateDefaultValue: function checkInDateDefaultValue() {
	            var checkInDate = new Date();
	            var _checkInDate = [checkInDate.getFullYear(), checkInDate.getMonth() + 1, checkInDate.getDate()].join("-");
	         ...
	         * 如无键盘弹起立即显示日期选择组件
	         */
	        handlerClickCheckInDate: function handlerClickCheckInDate(event) {
	         ...
	         * 设置选择日期给表单项
	         */
	        handlerKeyboardConfirm: function handlerKeyboardConfirm(event, value) {
	         ...
	         * 表单项: 是否看房 点击
	         */
	        handleClickIsSee: function handleClickIsSee(value) {
	            var _this = this;
	            if (value == 2) {
	         ...
	         */
	        handlerClickProtocol: function handlerClickProtocol() {
	            var _this = this;
	         ...
	         */
	        handlerDialogConfirm: function handlerDialogConfirm() {
	         ...
	         * 未看房显示警告提示框
	         */
	        createOrderBefore: function createOrderBefore() {
	            var _this = this;
	            //是否通过协议
	            if (_this.dataAgreement.value) {
	                //表单验证
	                var _validateResult = this.formValidate();
	                if (_validateResult.state) {
	                    var _formData = _this.formData;
	        ...
	        createOrder: function createOrder() {
	            var _this = this;
	            var _data = (0, _extends3.default)(this.formData, { infoId: _this.infoId });
	            _this.isLoading = true;
	            _this.$request.post({
	                url: _api2.default.createOrder.url,
	                data: _data,
	         ...
	         * 表单提交: 数据请求创建订单回调
	         */
	        createOrderCb: function createOrderCb(state, res) {
	            var _this = this;
	            _this.isLoading = false;
	            if (!state) {
	                this.$app.toastMsg("服务器接口异常");
	            ...
	            switch (+res.code) {
	                case 0:
	                    if (res.data) {
	                        var _loadPage = _this.loadPage.renterDetail;
	                        var _url = _loadPage.url + "?orderId=" + res.data.orderId;
	                ...
	                case 1000008:
	                    ...
	                    _this.$app.openFaceSdk("人脸认证", '3', function (state) {
	                        if (state == 0) {
	                            ...
	                            //取消认证操作
	                            _this.$app.showDialog("single", "提示", "您取消了人脸认证,请先认证再进行其他操作", function (index) {}, "确定");
	                        } else {
	    ...
	    components: {
	        PostHouse: _components.PostHouse,
	        PostInput: _components.PostInput,
	        PostRadio: _components.PostRadio,
	        PostSelect: _components.PostSelect,
	        DetailBottomBar: _components.DetailBottomBar,
	        DetailButton: _components.DetailButton,
	        KeyboardSelect: _components.KeyboardSelect,
	        AgreementRadio: _components.AgreementRadio,
	        DialogWarn: _components.DialogWarn
	    }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
	  return _c('article', {
	    ...
	    staticClass: "tips"
	  }, [_c('i'), _vm._v(_vm._s(_vm.orderTips))]), _vm._v(" "), _c('PostHouse', {
	    attrs: {
	  ...
	  }), _vm._v(" "), _c('PostSelect', {
	    attrs: {
	      "title": "入住日期",
	      "type": "data",
	      "name": "checkInDate",
	      "rules": _vm.rules.checkInDate,
	      "placeholder": "入住日期"
	    },
	    on: {
	      "click": _vm.handlerClickCheckInDate
	    },
	    model: {
	      value: (_vm.formData.checkInDate),
	      callback: function($$v) {
	        _vm.formData.checkInDate = $$v
	      },
	      expression: "formData.checkInDate"
	    }
	  })], 1), _vm._v(" "), _c('div', {
	    staticClass: "rowBox lineBottom"
	  }, [_c('PostInput', {
	    attrs: {
	      ...
	      "type": "tel",
	      "name": "buyerPhone",
	      "rules": _vm.rules.buyerPhone,
	      "placeholder": "填写您的手机号"
	    },
	    model: {
	      value: (_vm.formData.buyerPhone),
	      callback: function($$v) {
	        _vm.formData.buyerPhone = $$v
	      },
	      expression: "formData.buyerPhone"
	    }
	  })], 1), _vm._v(" "), _c('div', {
	    staticClass: "rowBox"
	  }, [_c('PostRadio', {
	    attrs: {
	      "name": "isSee",
	      "dataSource": _vm.dataSource.isSee,
	      "rules": _vm.rules.isSee
	    },
	    on: {
	      "click": _vm.handleClickIsSee
	    },
	    model: {
	      value: (_vm.formData.isSee),
	      callback: function($$v) {
	        _vm.formData.isSee = $$v
	      },
	      expression: "formData.isSee"
	    }
	  })], 1)]), _vm._v(" "), _c('AgreementRadio', {
	    on: {
	      "click": _vm.handlerClickProtocol
	    },
	    model: {
	  ...
	  }), _vm._v(" "), _c('DetailBottomBar', {
	    attrs: {
	      "data-position": _vm.dataBottomBar.barPosition
	    }
	  }, [_c('DetailButton', {
	    attrs: {
	      "data-button": _vm.dataButton
	    },
	    on: {
	      "click": _vm.handleClickButton
	    }
	  })], 1), _vm._v(" "), _c('DialogWarn', {
	    attrs: {
	      "text": _vm.dataDialog.text,
	      "button": _vm.dataDialog.cancelButton,
	      "next": _vm.dataDialog.confirmButton
	    },
	    on: {
	      "nextStep": _vm.handlerDialogConfirm
	    },
	    model: {
	      value: (_vm.dataDialog.show),
	      callback: function($$v) {
	        _vm.dataDialog.show = $$v
	      },
	      expression: "dataDialog.show"
	    }
	  }), _vm._v(" "), _c('KeyboardSelect', {
	    attrs: {
	      "show": _vm.keyboardShow,
	      "dataSource": _vm.dataSource.checkInDate,
	      "value": _vm.formData.checkInDate
	    },
	    on: {
	      "confirm": _vm.handlerKeyboardConfirm
	    }
	  }), _vm._v(" "), _c('Loading', {
	    attrs: {
	...
	if (false) {
	  module.hot.accept()
	  if (module.hot.data) {
	     require("vue-hot-reload-api").rerender("data-v-6fad3280", module.exports)
	  }
	}
